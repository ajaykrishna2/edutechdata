...
#Flagging the option chosen.On introduction of new steps add new elif
...
#Function To track number of devices  who accessed tara on whatsapp/portal.Creating Source Column and Metric Column.
def device_metric():
    ...
    li = [df1,df2, df3,df4];
    #Union Operation/Appending Data
    ...
    return df_pivot
##Function To Flag which menu user chose 
def menu_funnel():
...
menu   = menu_funnel();
df_merge = device.merge(menu,on=['Date','Source'],how='outer');
df_merge.fillna(0, inplace=True);
df_merge["Quiz"]=0
df_merge_slice = df_merge[['Date', 'Source',"Unique_Devices","Unique_Users_Clicked_On_Tara", 'Unique_Device_Clicked_On_Menu_Option_One', 'Unique_Device_Clicked_On_Menu_Option_Two',
         'Unique_Device_Clicked_On_Menu_Option_Three','Quiz','Unique_Device_Clicked_On_Menu_Option_Four',
         'Unique_Device_Clicked_On_Menu_Option_Five', 'Unique_Device_Clicked_On_Menu_Option_Six']]
df_merge_slice.sort_values("Source", axis=0, ascending=False, inplace=True, na_position='last')
print(df_merge_slice)
upload_gsheet(excel_file,"Main menu",df_merge_slice)
